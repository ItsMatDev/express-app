...
describe("POST /api/movies", () => {
  it("should return created movie", async () => {
    ...
    const [result] = await database.query("SELECT * FROM movies WHERE id=?", response.body.id);
  ...
  it("should return an error", async () => {
    ...
    expect(movieWithMissingProps).toHaveProperty("title");

    const response = await request(app).post("/api/movies").send(movieWithMissingProps);
...
describe("PUT /api/movies/:id", () => {
  ...
  it("should return an error", async () => {
    ...
    const response = await request(app).put(`/api/movies/1`).send(movieWithMissingProps);
...
describe("DELETE /api/movies/:id", () => {
  ...
  it("should return error", async () => {
    ...
    // if (result.some((e) => e.id === req.params.id)) {
    //   expect(response.status).toEqual(204);
    // } else {
    //   expect(response.status).toEqual(404);
    // }
